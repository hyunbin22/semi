...
public class LectureListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int totalLecture=service.countLectureApproval();
		...
		while(!(pageNo>pageEnd||pageNo>totalPage)) {
			if(pageNo==cPage) {
				...
				pageBar+="<a href='"+request.getContextPath()+"/lecture/lectureList?cPage="+pageNo+"'>"+pageNo+"</a>";
			}
			pageNo++;
		}
		...
		if(pageNo>totalPage) {
			...
			+"/lecture/lectureList?cPage="+(pageNo)+"'>></a>";
		}
