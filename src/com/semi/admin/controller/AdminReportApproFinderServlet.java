 ...
 */
@WebServlet("/admin/reportApproFinder.do")
public class AdminReportApproFinderServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(pageNo==1) {
			...
			pageBar+="<a href="+request.getContextPath()+"/admin/reportApproFinder.do?cPage="+(pageNo-1)+">[이전]</a>&nbsp;";
		}
		while(!(pageNo>pageEnd||pageNo>reportTotalPage)) {
			if(pageNo==cPage) {
				...
				pageBar+="<a href="+request.getContextPath()+"/admin/reportApproFinder.do?cPage="+pageNo+">"+pageNo+"</a>&nbsp;";
			}
			pageNo++;
		}
		if(pageNo>reportTotalPage) {
			...
			"/admin/reportApproFinder.do?cPage="+(pageNo)+">[다음]</a>";
		}
		...
		request.getRequestDispatcher("/views/admin/reportListFind.jsp").forward(request, response);
	}
